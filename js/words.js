document.addEventListener("DOMContentLoaded", function () {
    ...
    function splitPath (str) { return str.split('\\').pop().split('/').pop(); }
    var path = window.location.pathname;
    var fileName = splitPath(path);
    ...
    //Swedish
    case 'Swedish':
    $.getJSON("php/swedishWords.json", function (data) {
      swedishWords = JSON.parse(JSON.stringify(data));
     if (firstVisit === 'true') { 
    ...
    case 'English':
     $.getJSON("php/englishWords.json", function (data) {
      englishWords = JSON.parse(JSON.stringify(data));
      if (firstVisit == 'true') { 
    ...
    function GenerateWords(wordJSON, wordAmount) {
        ...
        var id = 1;
        var bigId = 0;
        // id behöver vara global och inte per letter array
    ...
    function GenerateMultiWords(wordJSON, wordAmount) {
        var wordsArray = [];
        var lettersArray = [];
        var bigArray = [];
        var bigHTML = '';
        ...
        for (i = 0; i < wordAmount; i++) {
            if (i == 0) {
                stringOfWords = wordJSON[i].word;
            } else {
                stringOfWords = stringOfWords + ' ,' + wordJSON[i].word
            }
        }
        console.log('string: ' + stringOfWords);
        wordsArray = stringOfWords.split(",");
...
document.getElementById('AltRight').classList.remove('pressed');
  ...
  case 110:
   ...
   $.ajax({
        ...
        method: 'POST',
        data: {
        userLogin: userLogin, userOne: userOne, userTwo: userTwo, userProgress: ProgScore
        },
